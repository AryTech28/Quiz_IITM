...
def update_db_schema():
    ...
    # Add this in update_db_schema() function
    try:
    ...
    try:
        ...
        cursor.execute("ALTER TABLE chapters ADD COLUMN description TEXT DEFAULT ''")
...
def add_subject():
    ...
    return render_template('add_subjects.html', error=error)
...
def add_chapter(subject_id):
    ...
    # Fetch the subject name to show in the template (optional, improves UX)
    subject_name = ""
    try:
        conn = sqlite3.connect('quiz_master.db')
        cursor = conn.cursor()
        ...
        pass  # Fallback in case subject fetch fails

    return render_template('add_chapters.html', error=error, subject_id=subject_id, subject_name=subject_name)
